...
router.get('/careers', (req: Request, res: Response) => {
  response(
    false,
    'This is the endpoint to upload the careers in the database',
    res,
    200
    )
  })

router.post('/careers', ensureToken, async (req: Request, res: Response) => {
  const { body: { args }, token } = req
  const { type } = args
  const c = new Careers(args)
  ...
  jwt.verify(token as string, SECRETE_KEY, async (error, jwtData) => {
    if (error) {
      ...
      try {
        const result = await c.process(type)
