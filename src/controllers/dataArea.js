...
class DataArea {
  ...
  async store (args) {
    ...
    try {
      const areaStored = await DataAreaModel.find({})
        .sort({ code: -1 })
        .limit(1)
      let dataArea
      ...
      if(areaStored.length > 0) {
        ...
        dataArea = new DataAreaModel({ abstract, code, image, name })
      } else dataArea = new DataAreaModel({ abstract, image, name })
  ...
  async getAll () {
    try {
      const result = await DataAreaModel.find({}, { __v: false, _id: false })
  ...
  async update (args) {
    ...
    if(!abstract && !image) throw new Error("There's nothing to update!")
    else if(abstract && !image)
      try {
        await DataAreaModel.findOneAndUpdate(
          {
            name: { $eq: name }
          },
          {
    ...
    const result = await DataAreaModel.findOne({ name: { $eq: name } })
